...
type Config struct {
	...
	MaxBody                                                     int64
	StartupTimeout, HeaderTimeout, ShutdownTimeout              time.Duration
}

func DefaultConfig() Config {
	return Config{
		Listen: "127.0.0.1:9931", Backend: "127.0.0.1:9934", LlamaServer: "llama-server",
		ModelDir: "models", Context: 32768, GPULayers: 999, MaxBody: 32 << 20,
		StartupTimeout: 2 * time.Minute, HeaderTimeout: 2 * time.Minute, ShutdownTimeout: 10 * time.Second,
	}
}
...
func ConfigFromEnv() (Config, error) {
	...
	ints := []struct {
		k string
		p *int
	}{{"FAMILIAR_LLM_CONTEXT", &c.Context}, {"FAMILIAR_LLM_GPU_LAYERS", &c.GPULayers}}
	for _, x := range ints {
	...
	durs := []struct {
		k string
		p *time.Duration
	}{{"FAMILIAR_LLM_STARTUP_TIMEOUT", &c.StartupTimeout}, {"FAMILIAR_LLM_HEADER_TIMEOUT", &c.HeaderTimeout}, {"FAMILIAR_LLM_SHUTDOWN_TIMEOUT", &c.ShutdownTimeout}}
	for _, x := range durs {
		if v := os.Getenv(x.k); v != "" {
	...
	if !isLoopback(c.Listen) {
		return c, fmt.Errorf("listen address %q is not loopback (set only a loopback address)", c.Listen)
	}
	return c, nil
}
...
func New(c Config, logger *slog.Logger) (*Service, error) {
	...
	if c.Upstream == "" && !isLoopback(c.Backend) {
...
func (s *Service) serve(w http.ResponseWriter, r *http.Request) {
	...
	case "/ready":
		if s.mgr != nil && !s.mgr.ready() {
			http.Error(w, "local backend not running", 503)
			return
		}
		w.WriteHeader(204)
		return
...
func (m *backendManager) ready() bool {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.proc != nil && m.starting == nil
}
...
func (m *backendManager) start(parent context.Context) error {
	...
	// llama.cpp output is not safe to assume secret-free (prompts and headers may
	// be logged), so the proxy never copies child output into its own log.
...
func (m *backendManager) wait(cmd *exec.Cmd) {
	e := cmd.Wait()
	m.mu.Lock()
	if m.proc == cmd {
		m.proc = nil
	...
	m.mu.Unlock()
	if !closing {
		m.log.Warn("local backend exited", "error", safeError(e))
	}
}
func (m *backendManager) stop(cmd *exec.Cmd) {
	if cmd.Process != nil {
