...
# From the repository root, the canonical Linux four-child deployment is:
./familiar.sh server
...
proxy upstream variables; children receive the stable local proxy URLs. Without an upstream, LLM and STT model downloads are staged
...
ready. Familiar Pi is deliberately not a supervisor child; systemd owns it.
...
configured consecutive-failure threshold is reached, processes are terminated
and restarted through the same
