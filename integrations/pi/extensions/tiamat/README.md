...
- A UI bridge in the same process (familiar-ui) can read the registered providers without touching the network: emit `familiar:tiamat:discover` with `{ accept(port) }` and the extension hands back a `TiamatPort` (`usage.ts`) whose `providers()` projects, per registered Tiamat provider, only fields the router publishes — `kind`, `locality`, the Pi provider ids registered for it, every catalogue model with its `availability`/`reason`/`resetsIn` (including unavailable ones Pi never registers), and `usage` as windows, spend or credits (or `null` for `usage: {}`). `familiar:tiamat:changed` fires after each catalog reconcile and usage poll. The token and base URL never leave the extension.
