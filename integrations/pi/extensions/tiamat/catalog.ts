  ...
  return value.every((record) => {
    ...
    return (
      ...
      (item.context_window === undefined ||
        (Number.isInteger(item.context_window) &&
          Number(item.context_window) > 0)) &&
      (item.max_output_tokens === undefined ||
        (Number.isInteger(item.max_output_tokens) &&
          Number(item.max_output_tokens) > 0)) &&
      ...
      (item.thinking_level_map === undefined ||
        (item.thinking_level_map !== null &&
          typeof item.thinking_level_map === "object")) &&
      (item.force_adaptive_thinking === undefined ||
        typeof item.force_adaptive_thinking === "boolean")
    );
  });
}
 ...
 */
export function catalogToProviderGroups(
  ...
  for (const record of catalog) {
    ...
    group.models.push({
      ...
      ...(record.thinking_level_map
        ? { thinkingLevelMap: record.thinking_level_map }
        : {}),
      ...(record.force_adaptive_thinking === undefined
        ? {}
        : {
            compat: { forceAdaptiveThinking: record.force_adaptive_thinking },
          }),
    });
  }
...
export function etagRequiresFetch(
  status: number,
...
export function withoutMaxOutputTokens(payload: unknown): unknown {
  if (
    ...
    return payload;
  const { max_output_tokens: _unsupported, ...rest } = payload as Record<
    string,
    unknown
  >;
  return rest;
