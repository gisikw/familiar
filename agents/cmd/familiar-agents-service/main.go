...
func main() {
	db := flag.String("db", env("FAMILIAR_AGENTS_DB", "agents.db"), "SQLite database path")
	socket := flag.String("unix", env("FAMILIAR_AGENTS_SOCKET", "agents.sock"), "Unix socket; empty disables")
	listen := flag.String("listen", env("FAMILIAR_AGENTS_LISTEN", "127.0.0.1:7337"), "loopback HTTP address; empty disables")
	flag.Parse()
