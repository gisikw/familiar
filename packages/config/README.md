...
Canonical typed model and Bun TOML loader for `familiar.toml`. It validates documented tables, applies defaults, then applies `FAMILIAR_*` environment overrides. Ambient variables win; `[familiar]` does not double the prefix. Files default to optional but, when present, must be mode `0600`.
