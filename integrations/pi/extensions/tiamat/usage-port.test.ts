      ...
      windows: [
        {
          ...
          used: "7%",
          resetsIn: "25m",
          resetsInSeconds: 1500,
        },
        {
          name: "weekly",
          used: "20%",
          resetsIn: "4d2h",
          resetsInSeconds: 352_800,
        },
      ],
    },
  },
      ...
      windows: [
        {
          ...
          used: "41%",
          resetsIn: "3d",
          resetsInSeconds: 259_200,
        },
      ],
      credits: { balance: "4.20", hasCredits: true },
      planType: "plus",
    },
  },
  "openrouter-personal": {
    ...
    locality: "remote",
    usage: {
      spend: { period: "month", amount: "12.47", currency: "USD" },
      fetchedAt: "2026-09-11T10:01:00.000Z",
    },
  },
...
describe("Tiamat UI port projection", () => {
  ...
  test("metered spend is a figure with no denominator, and a provider with no catalogue still appears", () => {
    ...
    expect(or.usage).toEqual({
      windows: [],
      spend: { period: "month", amount: "12.47", currency: "USD" },
      fetchedAt: Date.parse("2026-09-11T10:01:00.000Z"),
    });
    expect(or.models).toEqual([]);
  });

  test("tolerates a missing usage body and unparsable percentages", () => {
    const out = projectProviders(
      {
        odd: {
          usage: {
            windows: [
              { name: "w", used: "lots", resetsIn: "?", resetsInSeconds: 1 },
            ],
          },
        },
      } as any,
    ...
    expect(out[0]!.usage?.windows[0]).toEqual({
      name: "w",
      used: null,
      usedText: "lots",
      resetsIn: "?",
