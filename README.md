...
Herdr pane resolves the same fail-closed `pi` launcher for Familiar's patched
Pi 0.85.1, the pinned Herdr 0.9.1 CLI, and the worker tools the resident/Agents shells already use
...
credential provisioning stay fleet-side. The fleet-only launcher requires a
Tiamat URL and a readable, regular, nonempty token file before execing patched
Pi; resident Pi and the patched package remain directly invocable. See
