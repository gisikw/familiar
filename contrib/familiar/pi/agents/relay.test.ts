...
describe("direct-receipt ack", () => {
  test("reading a settled job's response withdraws the queued settlement item", async () => {
    const { client, jobs } = fakeClient();
    let sinkRef: DurableSink | undefined;
    const stateDir = newDir();
    jobs.set("d", settled("d", "done", "shipped"));
    // Sink down at settle time → pending retained, nothing surfaced yet.
    const relay = new SettlementRelay({ client, stateDir, resolveSink: () => sinkRef });
    await relay.recordDispatch("d");
    expect(readdirSync(path.join(stateDir, "pending")).length).toBe(1);
    // The operator/agent opens the job directly — that IS the receipt.
    const { sink, seen, withdrawn } = fakeSink();
    sinkRef = sink;
    await relay.noteDirectReceipt("d", jobs.get("d"));
    // The retained envelope is discarded, never enqueued, and tombstoned.
    expect(seen.size).toBe(0);
    expect(withdrawn).toContain("golem-settle-d");
    expect(readdirSync(path.join(stateDir, "pending")).length).toBe(0);
    expect(existsSync(path.join(stateDir, "done", "d.json"))).toBe(true);
    // Later reconciliation cannot resurrect it.
    await relay.flushPending();
    await relay.reconcileOwnedUnsettled();
    expect(seen.size).toBe(0);
    await relay.stop();
  });

  test("already-relayed settlement is withdrawn on direct receipt (undelivered only)", async () => {
    const { client, jobs } = fakeClient();
    const { sink, seen, withdrawn } = fakeSink();
    jobs.set("r", settled("r", "failed", "boom"));
    const relay = new SettlementRelay({ client, stateDir: newDir(), resolveSink: () => sink });
    await relay.recordDispatch("r"); // relayed → sink holds golem-settle-r
    expect(seen.has("golem-settle-r")).toBe(true);
    await relay.noteDirectReceipt("r", jobs.get("r"));
    // Withdrawal is requested; the sink (worklist) decides — it refuses when the
    // item was already delivered, so a surfaced nudge still needs an explicit ack.
    expect(withdrawn).toContain("golem-settle-r");
    await relay.stop();
  });

  test("non-terminal job: direct receipt is a no-op (blocked question survives)", async () => {
    const { client, jobs } = fakeClient();
    const { sink, seen, withdrawn } = fakeSink();
    jobs.set("b", blocked("b"));
    const stateDir = newDir();
    const relay = new SettlementRelay({ client, stateDir, resolveSink: () => sink });
    await relay.recordDispatch("b"); // blocked question enqueued
    const itemId = blockedItemId("b", jobs.get("b")!.question!);
    expect(seen.has(itemId)).toBe(true);
    // Reading the blocked job is not answering it: nothing is withdrawn.
    await relay.noteDirectReceipt("b", jobs.get("b"));
    expect(withdrawn).toEqual([]);
    expect(existsSync(path.join(stateDir, "blocked", "b.json"))).toBe(true);
    expect(existsSync(path.join(stateDir, "done", "b.json"))).toBe(false);
    await relay.stop();
  });

  test("unowned job: direct receipt neither claims nor tombstones it", async () => {
    const { client, jobs } = fakeClient();
    const { sink, withdrawn } = fakeSink();
    jobs.set("x", settled("x"));
    const stateDir = newDir();
    const relay = new SettlementRelay({ client, stateDir, resolveSink: () => sink });
    await relay.noteDirectReceipt("x", jobs.get("x")); // never dispatched by us
    expect(withdrawn).toEqual([]);
    expect(existsSync(path.join(stateDir, "done", "x.json"))).toBe(false);
    await relay.stop();
  });

  test("drop-box path: direct receipt removes an undrained settlement drop", async () => {
    const dropboxDir = newDir();
    const { client, jobs } = fakeClient();
    jobs.set("d", settled("d", "done", "quiet"));
    const relay = new SettlementRelay({ client, stateDir: newDir(), resolveSink: () => undefined, dropboxDir });
    await relay.recordDispatch("d"); // settles via the drop-box
    const drop = path.join(dropboxDir, "golem-settle-d.json");
    expect(existsSync(drop)).toBe(true);
    await relay.noteDirectReceipt("d", jobs.get("d"));
    expect(existsSync(drop)).toBe(false); // undrained drop pulled
    await relay.stop();
  });
});

describe("blocked-question drop-box", () => {
  test("drop written when sink absent, removed on unblock", async () => {
