...
export default function(pi:ExtensionAPI){const api=new GolemClient();
 ...
 pi.registerTool({name:"agents_status",label:"Agent Status",description:"List jobs or inspect one. Blocked job output includes its question.",parameters:Type.Object({id:Type.Optional(Type.String()),state:Type.Optional(Type.String())}),execute:(_id,p:any)=>run(()=>p.id?api.status(p.id):api.list(p.state)),...renderers("agents_status")});
