...
import { buildDispatchParameters, dispatchDescription, dispatchWorkspace, prepareDispatchArguments } from "./dispatch.ts";
...
const dispatchParameters=buildDispatchParameters(Type);

export default function(pi:ExtensionAPI){const api=new GolemClient();
 const acknowledgeTerminal=async(value:any)=>{if(!relay)return;const jobs=Array.isArray(value)?value:[value];for(const job of jobs){const state=job?.settlement?.state||job?.state;if(typeof job?.id==="string"&&["done","failed","cancelled","timeout"].includes(state))await relay.acknowledgeSettlement(job.id)}};
 ...
 // isolates the registry, it falls back to the familiar-services socket.
 ...
 const relay=stateDir?new SettlementRelay({client:api,stateDir,serviceSocket:process.env.FAMILIAR_SERVICES_SOCKET,resolveSink:()=>registry.resolve<DurableSink>(WORKLIST_SINK,WORKLIST_SINK_VERSION)}):undefined;
 ...
 pi.registerTool({name:"agents_dispatch",label:"Dispatch Agent",description:dispatchDescription,parameters:dispatchParameters,constrainedSampling:false,prepareArguments:prepareDispatchArguments,execute:(_id,p:any)=>run(async()=>{const workspace=dispatchWorkspace(p.workspace,p.worktree);return api.dispatch({harness:p.harness,model:p.model,workspace,prompt:p.prompt,idempotency_key:p.key||randomUUID()}).then(async(job:any)=>{const id=job?.id;if(relay&&typeof id==="string"&&id)await relay.recordDispatch(id);await acknowledgeTerminal(job);return job})}),...renderers("agents_dispatch")});
 pi.registerTool({name:"agents_status",label:"Agent Status",description:"List jobs or inspect one. Blocked job output includes its question. Lists return the 5 most recent jobs; use offset to page into older ones.",parameters:Type.Object({id:Type.Optional(Type.String()),state:Type.Optional(Type.String()),offset:Type.Optional(Type.Number({description:"Skip this many most-recent jobs before returning the next 5",minimum:0}))}),execute:(_id,p:any)=>run(async()=>{if(p.id){const result=await api.status(p.id);await acknowledgeTerminal(result);return result}const all=await api.list(p.state);const jobs=Array.isArray(all)?[...all]:[];jobs.sort((a:any,b:any)=>String(b?.created_at||"").localeCompare(String(a?.created_at||"")));const offset=Math.max(0,Math.floor(p.offset||0));const page=jobs.slice(offset,offset+5);await acknowledgeTerminal(page);return{total:jobs.length,offset,limit:5,jobs:page}}),...renderers("agents_status")});
 ...
 pi.registerTool({name:"agents_artifacts",label:"List Agent Artifacts",description:"List retained job artifacts.",parameters:Type.Object({id:Type.String()}),execute:(_id,p:any)=>run(()=>api.artifacts(p.id)),...renderers("agents_artifacts")});
 pi.registerTool({name:"agents_artifact_fetch",label:"Fetch Agent Artifact",description:"Fetch an artifact as base64 (maximum 2 MiB).",parameters:Type.Object({id:Type.String(),path:Type.String()}),execute:(_id,p:any)=>run(async()=>{const b=await api.fetchArtifact(p.id,p.path);if(b.byteLength>2*1024*1024)throw new Error("artifact exceeds 2 MiB tool limit");return{id:p.id,path:p.path,encoding:"base64",data:Buffer.from(b).toString("base64")}}),...renderers("agents_artifact_fetch")});
}
