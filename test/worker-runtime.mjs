...
// 5. Extension tree: the tiamat modules plus what they import, byte-equal to
// source, with every relative import resolvable inside the shipped tree.
...
assert.ok(shipped["tiamat/index.ts"], "tiamat entrypoint shipped");
assert.ok(!Object.keys(shipped).some((n) => n.endsWith(".test.ts")), "no tests shipped");
for (const [name, source] of Object.entries(shipped)) {
  assert.equal(source, readFileSync(join(extensionsRoot, name), "utf8"), `${name} differs from source`);
  for (const [, spec] of source.matchAll(/from\s+["'](\.{1,2}\/[^"']+)["']/g)) {
    const target = relative(shippedRoot, join(shippedRoot, dirname(name), spec));
    assert.ok(shipped[target] !== undefined, `${name} imports ${spec}, which is not shipped`);
  }
}
